...
public class OperationsUtil {
	...
	public static String doServletTradeStock(HttpServletRequest request, BigDecimal price) {

		User user = ServletUtil.getUser(request);
		String userName = user.getUsername();
		String direction = String.valueOf(request.getParameter("action"));
		String stockticker = request.getParameter("stockName");
		int shareNum = Integer.parseInt(request.getParameter("tradeAmount"));
		...
		if (creditActId < 0){
			message0 = "Destination account is invalid";
		}
		//if transfer amount is zero then there is nothing to do
		if (message0 == null){
			//Notice that available balance is not checked
			message = DBUtil.transferStock(userName, creditActId, stockticker, price, shareNum);
		}

		if (message0 != null){
			message0 = "ERROR: " + message0;
		} else {
			message1 = DBUtil.addHoldings(userName,stockticker,shareNum);
			message2 = DBUtil.addStockTransaction(userName, time, stockticker,  shareNum, price.doubleValue(), direction);
				if(direction.equals("sell")){
				message0 = "successfully sell " + stockticker + " at $" + price + " per share for $" + amount +  ". Time: " + time + ".";
			}
			else{
				message0 = "successfully buy " + stockticker + " at $" + price + " per share for $" + -amount +  ". Time: " + time + ".";
			}
		}
		return message0;
	}

	public static String addHistory(List<HistoricalQuote> stockHistQuotes) {
		String message = null;
		for (HistoricalQuote hist : stockHistQuotes)
		{
			String ticker = hist.getSymbol();
			SimpleDateFormat dateFormat = new SimpleDateFormat("yyyy-MM-dd");
			String date = dateFormat.format(hist.getDate().getTime());
			double price = hist.getAdjClose().doubleValue();
			message = DBUtil.addHistoryData(ticker, date, price);
			if (message != null) {
				return message;
			}
		}

		return "Add History Data to DB Successfully!";
	}
